    ...
    <g>
      ...
      <g>
        ...
        <path fill="#394240" d="M48,11c-0.553,0-1,0.447-1,1s0.447,1,1,1c4.418,0,8,3.582,8,8c0,0.553,0.447,1,1,1s1-0.447,1-1
          ...
          <div class="delete">
          <button id="delete${postId}" class="custom-btn btn-16">
            <span>Delete</span>
          </button>
        </div>
        </div>`;
...
async function deletepost(postId) {
  console.log(postId);
  if (!confirm("This post will be Deleted ?") == true) {
    return;
  }
  try {
    const { data, error } = await supabase.rpc("delete_gossipbar_post", {
      p_userid: user.id,
      p_postid: postId,
    });
...
async function fetchPost() {
  var dict = {};
  try {
    const { data, error } = await supabase.rpc("fetch_my_posts", {
      user_uuid: user.id,
      limit_count: 20,
      offset_count: c_of,
    });
